...
class License():
    ...
    def get_max_len(self):
        self.max_len = \
            max({'permissions': self.permissions, 'conditions': self.conditions,
...
class GitHub(Git, Repository):
    ...
    def get_repository_license(self, repository_owner, repository_name):
        response = requests.get(f'https://api.github.com/repos/{repository_owner}/{repository_name}/license',
                                headers={"Authorization": "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"})
        if response.status_code == 200:
            license_resp = requests.get(response.json()['license']['url']).json() if response.json()['license'][
                'url'] else None
            return License(license_resp['name'], license_resp['permissions'], license_resp['conditions'],
                           license_resp['limitations']) if license_resp else None
...
class GitLab(Git, Repository):
    ...
    def get_repository_license(self, repository_owner, repository_name):
        response = requests.get(f'https://gitlab.com/api/v4/projects/{repository_owner}%2F{repository_name}',
                                params={'license': True})
        if response.status_code == 200:
            license_resp = requests.get(
                f'https://gitlab.com/api/v4/licenses/{response.json()["license"]["key"]}').json() if response.json()[
                'license'] else None
            return License(license_resp['name'], license_resp['permissions'], license_resp['conditions'],
                           license_resp['limitations']) if license_resp else None
        elif response.status_code == 404:
